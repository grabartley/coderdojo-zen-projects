...
docker build --rm=false --build-arg DEP_VERSION=$DEP_VER -t coderdojo/cp-organisations-service:"$GIT_SHA1" .
docker login -u "$DOCKER_USER" -p "$DOCKER_PASS" -e "$DOCKER_EMAIL"
docker push coderdojo/cp-organisations-service:"$GIT_SHA1"
...
./kubectl patch deployment organisations -p '{"spec":{"template":{"spec":{"containers":[{"name":"organisations","image":"coderdojo/cp-organisations-service:'"$GIT_SHA1"'"}]}}}}'
