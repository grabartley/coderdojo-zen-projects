...
docker build --rm=false --build-arg DEP_VERSION=$DEP_VER -t coderdojo/cp-eventbrite-service:"$GIT_SHA1" .
docker login -u "$DOCKER_USER" -p "$DOCKER_PASS" -e "$DOCKER_EMAIL"
docker push coderdojo/cp-eventbrite-service:"$GIT_SHA1"
...
./kubectl patch deployment eventbrite -p '{"spec":{"template":{"spec":{"containers":[{"name":"eventbrite","image":"coderdojo/cp-eventbrite-service:'"$GIT_SHA1"'"}]}}}}'
